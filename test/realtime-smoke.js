...
const base = process.env.C9_TEST_WS || "ws://127.0.0.1:1337";
...
function login() {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request("http://127.0.0.1:1337/login", {
      method: "POST",
...
function connect(pathname, cookie) {
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(base + pathname, { headers: { Cookie: cookie } });
