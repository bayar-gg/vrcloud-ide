...
class AnthropicAgent {
  ...
  async runTool(name, input, callId, signal) {
    ...
    if (this.customTools[name]) {
      const r = await this.customTools[name].execute(input, { toolCallId: callId });
    ...
    switch (lname) {
      ...
      case "updatetodos": {
        ...
        let out;
        if (input.merge && this.todos && this.todos.length) {
          out = this.todos.map((t) => ({ content: t.content, status: t.status }));
