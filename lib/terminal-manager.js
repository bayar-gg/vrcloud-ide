...
class TerminalManager {
  ...
  create(cwd) {
    ...
    if (!existed) {
      const command = "exec bash --rcfile " + JSON.stringify(this.rcfile) + " -i";
      const made = spawnSync("tmux", ["new-session", "-d", "-s", name, "-c", meta.cwd || this.workspace, command]);
  ...
  attach(ws, id) {
    ...
    if (history && ws.readyState === WebSocket.OPEN) ws.send(history);
    ws.on("message", (raw) => {
