 ...
 *
 * Provider: Cursor SDK (default) atau Anthropic Claude langsung (lib/anthropic-agent.js,
 * antarmuka yang sama sehingga UI/kartu/checkpoint identik).
 *
 * Env:
 *   CURSOR_API_KEY     kunci API Cursor
 *   ANTHROPIC_API_KEY  kunci API Anthropic (Claude)
 *   AI_PROVIDER        "cursor" | "anthropic" (default: otomatis dari key yang ada)
...
class AiChat {
  ...
  recompute() {
    ...
    // Provider: pilihan web > AI_PROVIDER > otomatis (key yang tersedia).
    const pv = String(this.saved.provider || this.envProvider || "").toLowerCase();
    if (pv === "anthropic" || pv === "cursor") this.provider = pv;
    else this.provider = this.apiKey ? "cursor" : (this.anthropicKey ? "anthropic" : "cursor");
  ...
  async setConfig(config) {
    ...
    if (Object.prototype.hasOwnProperty.call(config, "provider")) {
      ...
      if (["", "cursor", "anthropic"].indexOf(p) === -1) throw new Error("Provider tidak dikenal: " + p);
  ...
  status() {
    ...
    const hasSdk = isAnth ? true : !!sdk;
    const hasKey = isAnth ? !!this.anthropicKey : !!this.apiKey;
    const source = isAnth
    ...
    return {
      ...
      source, keyMasked: mask(isAnth ? this.anthropicKey : this.apiKey), reason: rd.reason,
  ...
  agentPrompt(userText, priorTranscript) {
    ...
    return Instructions.agentPrompt({
      ...
      includeStatic: this.provider !== "anthropic",
  ...
  async send(sessionId, message, onEvent, extra) {
    ...
    try {
      ...
      // Provider tanpa memori sisi server (Anthropic): simpan riwayat percakapan ringkas.
      if (s.agent && typeof s.agent.getHistory === "function") {
        try { meta.claudeHistory = s.agent.getHistory(); } catch (e) {}
      }
      const live = s.live || {};
